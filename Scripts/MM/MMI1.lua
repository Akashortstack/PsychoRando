...
function MMI1(Ob)
	if not Ob then
		...
		Ob.dependencies = {
			...
			textures = {
				'Textures/icons/PsiPowers/Clairvoyance.dds',
				'textures/fxtextures/clairvoyance/staticline.dds',
				'textures/fxtextures/clairvoyance/bars.dds',
				'textures/ui/keypad/mm_keypad_base.dds',
				'textures/ui/keypad/mm_keypad_button_on.dds',
				'textures/ui/keypad/mm_keypad_button_off.dds',
				'textures/ui/keypad/mm_keypad_shine.dds',
				'textures/ui/keypad/mm_keypad_big_button_off.dds',
				'textures/ui/keypad/mm_keypad_big_button_on.dds',
				'textures/leveltextures/mm_milkmanconspiracy/mm_footprint_right.dds',
				'textures/leveltextures/mm_milkmanconspiracy/mm_footprint_left.dds',
			},
			animations = {
				'Anims/AS_Feather/idle.jan',
				'Anims/AS_Feather/settle.jan',
				'Anims/BlackSedan/Damage.jan',
				'Anims/BlackSedan/TrunkOpen.jan',
				...
				'Anims/DartNew/Clippers_UseClippers_end.jan',
				'Anims/DartNew/Clippers_UseClippers_loop.jan',
				'Anims/DartNew/Clippers_UseClippers_start.jan',
				...
				'Anims/DartNew/UseRollingPin.jan',
				...
				'Anims/Gman/Clippers/ArrestSequence_Walk.jan',
				'Anims/Gman/Clippers/ArrestSequence_Idle.jan',
				'Anims/Gman/Clippers/Clipping.jan',
				'Anims/Gman/Clippers/Clippers_Walk.jan',
				'Anims/Gman/Clippers/Flying.jan',
				'Anims/Gman/Clippers/Swallow.jan',
				'Anims/Gman/Keypad.jan',
				'Anims/Gman/WaterCanBalance.jan',
				'Anims/Gman/WaterCanWatering1.jan',
				'Anims/Gman/WaterCanWatering2.jan',
				'Anims/Gman/WaterCanYorick.jan',
				'Anims/MM_Cameras/mailbox_Firestarting.jan',
				'Anims/MM_Cameras/mailbox_Flinch.jan',
				'Anims/MM_Cameras/mailbox_Confusion.jan',
				'Anims/MM_Cameras/mailbox_doorOpen.jan',
				'Anims/MM_Cameras/mailbox_tremble.jan',
				'Anims/MM_Cameras/mailbox_walk.jan',
				'Anims/MM_HedgeMaze/TurstileBackOpen.jan',
				'Anims/MM_HedgeMaze/TurstileFrontClose.jan',
				'Anims/MM_HedgeMaze/TurstileFrontOpen.jan',
				'Anims/MM_HedgeMaze/TurstileBackClose.jan',
				'Anims/Objects/InventoryToss.jan',
				'Anims/Objects/InventoryToss_largeobjects.jan',
				'Anims/Rainbow_Squirts/Whisper.jan',
...
-- ****************************************************************************
	...
	function Ob:onBeginLevel()
		...
		-- #temp
		--self.cam = SpawnScript('MM.Props.SpyCamBoyd', 'BoydCamera', '')
		...
		RegisterTriggerVolume(self, 'tv_RazAbduction1a', 0)
		RegisterTriggerVolume(self, 'tv_RazAbduction1b', 0)
...
--		DomainSleep('DemonLsos', 1)
		...
		self.cookieBombFX = SpawnScript('MM.Effects.CookieBoxExplosionFX', 'CookieBombFX')
		...
		-- can't turn COLLIDER off in the class, because it's movinggeometry
		-- pld 1/2005
		for i=1,17 do
			local e = FindScriptObject(format('chopper%dChopperMesh', i))
			SetPhysicsFlag(e, PHYSICS_COLLIDER, 0)
		end
		
		-- kill manhole cover if we're coming from mmi2
		if (Global:loadGlobal('LastLevel') == 'MMI2') then
			local manhole = FindScriptObject('ManholeCover')
			if (manhole) then manhole:killSelf() end
		end
	end
	...
	function Ob:onPostBeginLevel()
		...
		local basementDomainController = FindScriptObject('basementDOMController')
		
		if (Global:load('MMIntroSeen') ~= 1) then
			Global:save('MMIntroSeen', 1)
			Global.cutsceneScript:runCutscene('IntroMM')
		end
		
		if (Global:load('bBoydClairPlayed') == 1) then
			FindScriptObject('BoydFakeWall'):killSelf()
			FindScriptObject('transexitboyds'):turnOn()
		else
			FindScriptObject('transexitboyds'):turnOff()
		end

		if Global:load('CemetaryDoorDestroyed') == 1 then
			SetSwitchableCollisionEnable(2, 0)
		else
			SetSwitchableCollisionEnable(2, 1)
			local cemDoor = FindScriptObject('CemetaryDoor')
			if not cemDoor then
				GamePrint('ERROR: CemetaryDoor not found.  It needs to be added to the level.')
				SetSwitchableCollisionEnable(2, 0)
			else
				local oldFunc = cemDoor.onDestroyed
				cemDoor.onDestroyed = function(self, bBecause)
					%oldFunc(self, bBecause)
					if bBecause ~= 1 then
						SetSwitchableCollisionEnable(2, 0)
					end
				end
		...
		self:setBasementClairEnabled(0)
	end
	...
	function Ob:revisitingSetup()
		...
		local k = FindScriptObject('Keypad')
		k:onKeypadEntry('Granted')
		...
		FindScriptObject('Boyd'):killSelf()
...
--    **********************************************************
	...
	function Ob:footfall(foot, runType)
		if self.insideBasement == 1 and self.currentClairedCamera then
...
--  M E S S A G E S  **********************************************************
	...
	function Ob:onEnteredTriggerVolume(tv, from)
    	...
    	elseif (tv == 'tv_RazAbduction1a') or (tv == 'tv_RazAbduction1b') then
		...
		elseif tv == 'tv_ClairvoyanceClueCS' then
			if Global.player.HeldItem ~= 'Sign' and (Global:load('bSignGmenClairvoyanceCluePlayed') ~= 1) then
				Global.cutsceneScript:runCutscene('SignGmenClairvoyanceClue')
				if (Global.goalManager:isActive('FindGraveyard') == 1) then
					Global.goalManager:activate('RoadCrew')
				end
			end
		elseif tv == 'tv_itemClairHint' then
			if Global:loadGlobal('bSeenClairvoyanceItemHint') ~= 1 then
				Global:saveGlobal('bSeenClairvoyanceItemHint', 1)
				Global.levelScript:addTutorialBox("/MMZF000TO/", 'Textures/icons/PsiPowers/Clairvoyance.dds')--DIALOG=<</Clairvoyance/ Sometimes you can use CLAIRVOYANCE on an object to see from the point of view of the owner. Just hold the item in your hand and use the CLAIRVOYANCE power. Items that have strong clairvoyance reaction will appear with a special aura in your inventory, like this: /clairvoyance aura/>>
			end
		else
			%Ob.Parent.onEnteredTriggerVolume(self, tv, from)
			-- for demon music
	...
	function Ob:onExitedTriggerVolume(tv, from)
		...
		elseif from == Global.player and tv == 'tv_Basement' then
			...
			RemoveAllDecals(nil, 'Textures/LevelTextures/MM_MilkmanConspiracy/MM_footprint_left.tga')
			RemoveAllDecals(nil, 'Textures/LevelTextures/MM_MilkmanConspiracy/MM_footprint_right.tga')
...
-- ****************************************************************************
	...
	function Ob:onKeypadEntry(data)
		--if not self.cutsceneScript.currCSName then
		--end
		if data ~= 'Granted' then return end
		local door = FindScriptObject('KeypadDoor')
		if door then
			door:open()
		else
			GamePrint('ERROR: no door available for keypad to open.')
		end
	end
...
-- ****************************************************************************
	...
	-- Enable/disable various visuals in the basement on clairvoyance.
	-- If self.currentClairedCamera ~= nil, that means we're in night vision mode.
	function Ob:setBasementClairEnabled(bEnabled, whichCamera)
		if bEnabled == 1 then
			self.currentClairedCamera = whichCamera
			local plunger = FindScriptObject('Plunger', 1)
			if plunger and Global.player:isInInventory(plunger.Name) ~= 1 then
				plunger:show()
			end
			SetClairvoyanceNightvision(1)
			SetStaticLightIntensity(1)
			SetDynamicLightIntensity(.2)
		elseif whichCamera == self.currentClairedCamera then
			self.currentClairedCamera = nil
			local plunger = FindScriptObject('Plunger', 1)
			if plunger and Global.player:isInInventory(plunger.Name) ~= 1 then
				plunger:hide()
			end
			if IsDomainSleeping('basementDOMAIN') ~= 1 then 
				SetStaticLightIntensity(0) 
			end
		end
	end
...
--*************************************************************************************************
	...
	function Ob:listenerRazRespawn()
		-- these cutscenes can happen anywhere, even when you're in midair and falling to your death.
		if Global.cutsceneScript.currCSName == 'SquirtInteraction' or Global.cutsceneScript.currCSName == 'SquirtAfterSniperDied' or Global.cutsceneScript.currCSName == 'SquirtIntro' then
			Global.cutsceneScript:onCutsceneOverride()
		end
	end
...
--
-- When you want to test this out, copy MMI1.lua over to the xbox
