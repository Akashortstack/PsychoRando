...
function Confusion(Ob)
	if not Ob then
		Ob = CreateObject('Global.Props.HeldObject')
		Ob.power = 'Confusion' -- Editable
		--edit for Confusion texture, using Clairvoyance Mesh as base
		...
		Ob.dependencies = {
			textures = {
				'Textures/icons/PsiPowers/Confusion.dds' --Since we load replace the texture after-the-fact, we tell the game we depend on it so it gets preloaded
			}
		}
	end	
	...
	function Ob:onBeginLevel()
		...
		--edit
		if Global.player.stats.RandoConfusion[self.Name] == 'collected' then
			self:killSelf()
		end
		...
		SetBaseTexture(self,'Textures/icons/PsiPowers/Confusion_LIT.dds')
	...
	function Ob:statePickup()
		...
		if self.power == 'Confusion' then
			...
			elseif (Global:loadGlobal('bConfusionUp') ~= 1) then
				Global:saveGlobal('bConfusionUp', 1)
			end
		end
