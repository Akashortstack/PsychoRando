...
function Shield(Ob)
	if not Ob then
		...
		Ob.dependencies = {
			textures = {
				'Textures/icons/PsiPowers/Shield.dds' --Since we load replace the texture after-the-fact, we tell the game we depend on it so it gets preloaded
			}
		}
	end	
	...
	function Ob:statePickup()
		...
		self:sleep(.5)
		--edit to play Custom Shield cutscene
		self:playVideo('cutScenes/prerendered/ShieldMeritBadge.bik')
		...
		if self.power == 'Shield' then
			...
			elseif (Global:loadGlobal('bSuperShield') ~= 1) then
				Global:saveGlobal('bSuperShield', 1)
			elseif (Global:loadGlobal('bDamageShield') ~= 1) then
				Global:saveGlobal('bDamageShield', 1)
			end
		end
		...
		self:sendMessage(Global.player, 'RandoShield', self.Name, 1)
	...
	function Ob:getConstant()
		...
		elseif self.power == 'Invisibility' then
			...
			--edit adding Levitation as constant
		elseif self.power == 'Levitation' then
			constant = kPOWER_LEVITATION
		end
		return constant
	end
