function RandoSuitcase(Ob)
	...
	if (Ob == nil) then 	
		...
		Ob.sBaggageType = 'suitcase' -- Editable
		Ob.animTimerID = '1'
		Ob.dieTimerID = '2'
	...
	function Ob:onBeginLevel()
		...
		--edit
		if Global.player.stats.RandoSuitcase[self.Name] == 'collected' then
			self:killSelf()
		end
		...
		--edit
		self:loadMesh('GlobalModels/CO_Collectibles/CO_suitcase.plb')
		self:setScale(2)
		...
		if ( levelPrefix == 'BV' ) then
			--edit
			SetBaseTexture(self, 'Textures/Collectibles/co_suitcase_BV.dds')
		end
		SetActionTable(self, 'Anims/Emotional Baggage/suitcase.asd')
