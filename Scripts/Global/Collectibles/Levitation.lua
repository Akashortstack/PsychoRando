function Levitation(Ob)
	...
	function Ob:onBeginLevel()
		...
		--edit
		if Global.player.stats.RandoLevitation[self.Name] == 'collected' then
			self:killSelf()
		end
		...
		SetBaseTexture(self,'Textures/icons/PsiPowers/Levitation_LIT.dds')
	...
	function Ob:statePickup()
		...
		if self.power == 'Levitation' then
			if (Global:loadGlobal('bGotRando'..self.power, 1) ~= 1) then
				Global:saveGlobal('bGotRando'..self.power, 1)
			elseif (Global:loadGlobal('bSpikeBall') ~= 1) then
				Global:saveGlobal('bSpikeBall', 1)
			elseif (Global:loadGlobal('bSuperSpikeBall') ~= 1) then
				Global:saveGlobal('bSuperSpikeBall', 1)
			end
		end
		...
		self:sendMessage(Global.player, 'RandoLevitation', self.Name, 1)
