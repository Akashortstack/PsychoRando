...
function Marksmanship(Ob)
	...
	function Ob:onBeginLevel()
		...
		--edit
		if Global.player.stats.RandoMarksmanship[self.Name] == 'collected' then
			self:killSelf()
		end
	...
	function Ob:statePickup()
		...
		self:sleep(.5)
		--edit to play correct video
		self:playVideo('cutScenes/prerendered/marksmanshipbadge.xmv')
		...
		if self.power == 'Marksmanship' then
			if (Global:loadGlobal('bGot'..self.power) ~= 1) then
				Global:saveGlobal('bGot'..self.power, 1)
			elseif (Global:loadGlobal('bChainBlast') ~= 1) then
				Global:saveGlobal('bChainBlast', 1)
			elseif (Global:loadGlobal('bSuperChainBlast') ~= 1) then
				Global:saveGlobal('bSuperChainBlast', 1)
			end
		end
		...
		self:sendMessage(Global.player, 'RandoMarksmanship', self.Name, 1)
	...
	function Ob:getConstant()
		...
		elseif self.power == 'Invisibility' then
			...
			--edit adding kPOWER_PSIBLAST as constant, 
		elseif self.power == 'Marksmanship' then
			constant = kPOWER_PSIBLAST
		end
		return constant
	end
