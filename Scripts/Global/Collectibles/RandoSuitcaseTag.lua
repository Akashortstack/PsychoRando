...
function RandoSuitcaseTag(Ob)
	...
	if (Ob == nil) then 	
		...
		Ob.sBaggageType = 'suitcase_tag' -- Editable
		Ob.invSortKey = 15
		--edit
		Ob.level = 'all'
	...
	function Ob:onBeginLevel()	
		...
		--edit
		if Global.player.stats.RandoSuitcaseTag[self.Name] == 'collected' then
			self:killSelf()
		end
        ...
        --edit        
		self.meshName = 'GlobalModels/Objects/HeldObjects/suitcase_tag.plb'
		...
		if strfind(FindScriptObject('LevelScript').Type, 'BV') then
			--edit
			SetBaseTexture(self, 'Textures/Collectibles/co_suitcase_tag_BV.dds')
		end
