function RandoSteamertrunk(Ob)
	...
	if (Ob == nil) then 	
		...
		Ob.sBaggageType = 'steamertrunk' -- Editable
		Ob.animTimerID = '1'
		Ob.dieTimerID = '2'
	...
	function Ob:onBeginLevel()		  
		...
		--edit
		if Global.player.stats.RandoSteamertrunk[self.Name] == 'collected' then
			self:killSelf()
		end
		...
		--edit
		self:loadMesh('GlobalModels/CO_Collectibles/CO_steamertrunk.plb')
		self:setScale(2)
		...
		if ( levelPrefix == 'BV' ) then
			--edit
			SetBaseTexture(self, 'Textures/Collectibles/co_steamertrunk_BV.dds')
		end
		SetActionTable(self, 'Anims/Emotional Baggage/steamertrunk.asd')
