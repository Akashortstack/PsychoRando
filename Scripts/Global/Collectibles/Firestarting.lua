function Firestarting(Ob)
	...
	function Ob:statePickup()
		...
		if self.power == 'Firestarting' then
			...
			elseif (Global:loadGlobal('bPKDetonate') ~= 1) then
				Global:saveGlobal('bPKDetonate', 1)
			end
		end
		...
		self:sendMessage(Global.player, 'RandoFirestarting', self.Name, 1)
