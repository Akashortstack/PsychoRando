function RandoDufflebag(Ob)
	...
	if (Ob == nil) then 	
		...
		Ob.sBaggageType = 'dufflebag' -- Editable
		Ob.animTimerID = '1'
		Ob.dieTimerID = '2'
	...
	function Ob:onBeginLevel()
		...
		--edit
		if Global.player.stats.RandoDufflebag[self.Name] == 'collected' then
			self:killSelf()
		end
		...
		--edit
		self:loadMesh('GlobalModels/CO_Collectibles/CO_dufflebag.plb')
		self:setScale(2)
		...
		if ( levelPrefix == 'BV' ) then
			--edit
			SetBaseTexture(self, 'Textures/Collectibles/co_dufflebag_BV.dds')
		end
		SetActionTable(self, 'Anims/Emotional Baggage/dufflebag.asd')
	...
	function Ob:stateActivated()
		--edit stat type
		if (Global.player.stats.CollectedDufflebagTag >= 1) then -- player has collected our matching tag
			--edit to sendMessage to Dart
			self:sendMessage(Global.player, 'CollectedDufflebag', self.Name, 1)
