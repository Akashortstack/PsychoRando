...
function RandoHatboxTag(Ob)
	...
	if (Ob == nil) then 	
		...
		Ob.sBaggageType = 'hatbox_tag' -- Editable
		Ob.invSortKey = 15
		--edit
		Ob.level = 'all'
	...
	function Ob:onBeginLevel()	
		...
		--edit
		if Global.player.stats.RandoHatboxTag[self.Name] == 'collected' then
			self:killSelf()
		end
        ...
        --edit        
		self.meshName = 'GlobalModels/Objects/HeldObjects/hatbox_tag.plb'
		...
		if strfind(FindScriptObject('LevelScript').Type, 'BV') then
			--edit
			SetBaseTexture(self, 'Textures/Collectibles/co_hatbox_tag_BV.dds')
		end
	...
	function Ob:statePickup()
		...
		self:sendMessage(Global.player, 'CollectedHatboxTag', self.Name, 1)
