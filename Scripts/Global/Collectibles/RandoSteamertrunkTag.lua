...
--edit name    
function RandoSteamertrunkTag(Ob)
	...
	if (Ob == nil) then 	
		...
		Ob.sBaggageType = 'steamertrunk_tag' -- Editable
		Ob.invSortKey = 15
		--edit
		Ob.level = 'all'
	...
	function Ob:onBeginLevel()	
		...
		--edit
		if Global.player.stats.RandoSteamertrunkTag[self.Name] == 'collected' then
			self:killSelf()
		end
        ...
        --edit        
		self.meshName = 'GlobalModels/Objects/HeldObjects/steamertrunk_tag.plb'
		...
		if strfind(FindScriptObject('LevelScript').Type, 'BV') then
			--edit
			SetBaseTexture(self, 'Textures/Collectibles/co_steamertrunk_tag_BV.dds')
		end
	...
	function Ob:statePickup()
		...
		self:sendMessage(Global.player, 'CollectedSteamertrunkTag', self.Name, 1)
