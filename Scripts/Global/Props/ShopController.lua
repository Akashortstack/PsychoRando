...
--			end
			...
			for i = 1, maxitems do
				if (self.tItems[i]) then
					...
					--edit to check cobweb duster status, remove from shop if needed
					elseif self.tItems[i].Type == 'global.collectibles.CobwebDuster' then
						if Global.player:isInInventory('CobwebDuster') == 1 then
							self.tItems[i]:removeFromNeighbors()
							self.tItems[i] = nil
						elseif Global:loadGlobal('bCobwebRandomized') == 1 then
							self.tItems[i]:removeFromNeighbors()
							self.tItems[i]:killSelf()
							self.tItems[i] = nil
						end
