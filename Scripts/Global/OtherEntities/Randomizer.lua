function Randomizer(Ob)
    ...
    function Ob:placeCollectibles()
        if strfind(FindScriptObject('LevelScript').Type, 'ca.CAKC') then
            local item1 = SpawnScript('global.collectibles.Marksmanship');
            item1:setPosition(-5629, 4510, -13242);
            item1:setOrientation(0, 0, 0);
            GamePrint('Created RandoItem!')
        else
            GamePrint("ERROR, FAILED TO SPAWN Marksmanship IN LEVELSCRIPT ca.CAKC")
        end
    end
